...
SeriousBug::SeriousBug(const std::string& w, const CodeLocation& loc) :
    Exception("Serious bug: " + w, loc) {
...
BadParameter::BadParameter(const std::string& w, const CodeLocation& loc) :
    Exception("Bad parameter: " + w, loc) {}

BadCast::BadCast(const std::string& w, const CodeLocation& loc) :
    Exception("Bad cast: " + w, loc) {}

BadValue::BadValue(const std::string& w, const CodeLocation& loc) :
    Exception("Bad value: " + w, loc) {}

Stop::Stop(const std::string& w, const CodeLocation& loc) :
    Exception("Stop: " + w, loc) {}

Abort::Abort(const std::string& w, const CodeLocation& loc) :
    Exception("Abort: " + w, loc) {}

Cancel::Cancel(const std::string& w, const CodeLocation& loc) :
    Exception("Cancel: " + w, loc) {}

Retry::Retry(const std::string& w, const CodeLocation& loc) :
    Exception("Retry: " + w, loc) {}

UserError::UserError(const std::string& w, const CodeLocation& loc) :
    Exception("User error: " + w, loc) {}
...
OutOfRange::OutOfRange(unsigned long long index, unsigned long long max, const CodeLocation& loc) :
    Exception("Out of range: accessing element " + std::to_string(index) + ", but maximum is " + std::to_string(max - 1), loc) {}

OutOfRange::OutOfRange(const std::string& w, const CodeLocation& loc) :
    Exception("Out of range: " + w, loc) {}
...
CantOpenFile::CantOpenFile(const std::string& file, bool retry, const CodeLocation& loc) :
    retry_(retry) {
    std::ostringstream s;
