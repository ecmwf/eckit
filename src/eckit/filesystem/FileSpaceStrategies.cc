...
struct Candidate {
    ...
    long usedPercent() const { return long(100. * (double(size_.total - size_.available) / size_.total)); }
    long availablePercent() const { return long(100. * (double(size_.available) / size_.total)); }
