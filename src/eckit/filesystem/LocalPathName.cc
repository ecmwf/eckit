...
static void tidy_path(std::vector<std::string>& v) {
    ...
    while (more) {
        ...
        while (j != v.end()) {
            ...
            ++pass2;
//            Log::info() << "pass [" << pass1 << ',' << pass2 << "] -- v " << v << std::endl;
...
static void print_stack(const std::stack<std::string>& st) {
    std::stack<std::string> s = st;
...
static void tidy_path_stack(std::vector<std::string>& v) {
    ...
    bool absolute = false;
    if(v[0] == "") { // path started with / -> absolute path
        absolute = true;
    }
...
//        print_stack(path);
        ...
        if (s.empty()) { // ignore empty entries generated by '//'
            continue;
        }
