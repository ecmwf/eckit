/*
 * (C) Copyright 1996- ECMWF.
 *
 * This software is licensed under the terms of the Apache Licence Version 2.0
 * which can be obtained at http://www.apache.org/licenses/LICENSE-2.0.
 *
 * In applying this licence, ECMWF does not waive the privileges and immunities
 * granted to it by virtue of its status as an intergovernmental organisation nor
 * does it submit to any jurisdiction.
 */


#pragma once

#include "eckit/geo/grid/RegularGrid.h"


namespace eckit::geo::grid {


class LambertAzimuthalEqualArea : public RegularGrid {
public:
    // -- Exceptions
    // None

    // -- Constructors

    LambertAzimuthalEqualArea(const Configuration&);
    LambertAzimuthalEqualArea(const Projection&, const BoundingBox&, const LinearSpacing& x,
                              const LinearSpacing& y, const Shape&);
    LambertAzimuthalEqualArea(const LambertAzimuthalEqualArea&) = delete;

    // -- Destructor
    // None

    // -- Convertors
    // None

    // -- Operators

    LambertAzimuthalEqualArea& operator=(const LambertAzimuthalEqualArea&) = delete;

    // -- Methods
    // None

    // -- Overridden methods
    // None

    // -- Class members
    // None

    // -- Class methods
    // None

private:
    // -- Members
    // None

    // -- Methods

    static Projection make_projection(const Configuration&);

    // -- Overridden methods
    // None

    // -- Class members
    // None

    // -- Class methods
    // None

    // -- Friends
    // None
};


}  // namespace eckit::geo::grid
