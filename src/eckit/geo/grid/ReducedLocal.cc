...
size_t ReducedLocal::size() const {
    return niacc().back();
}


std::vector<Point> ReducedLocal::to_points() const {
...
std::pair<std::vector<double>, std::vector<double>> ReducedLocal::to_latlon() const {
    const auto N = size();
...
ReducedLocal::ReducedLocal(const Spec& spec) :
    Grid(spec) {}


ReducedLocal::ReducedLocal(const area::BoundingBox& bbox) :
    Grid(bbox) {}


const std::vector<size_t>& ReducedLocal::niacc() const {
    if (niacc_.empty()) {
        niacc_.resize(1 + nj());
        niacc_.front() = 0;
