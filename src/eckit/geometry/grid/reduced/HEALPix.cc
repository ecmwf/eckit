...
#include "eckit/geometry/grid/reduced/HEALPix.h"
...
Grid::iterator HEALPix::cbegin() const {
    return ordering_ == Ordering::healpix_ring ? iterator{new geometry::iterator::Reduced(*this, 0)} : NOTIMP;
}


Grid::iterator HEALPix::cend() const {
    return ordering_ == Ordering::healpix_ring ? iterator{new geometry::iterator::Reduced(*this, size())} : NOTIMP;
}
