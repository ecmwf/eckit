...
MoverHandle::MoverHandle(DataHandle* handle) : handle_(handle), fail_(false) {}
...
void MoverHandle::close() {
    ...
    data_.close();
    if (fail_)
        throw Exception(std::string("MoverHandle::close: ") + error_);
}

long MoverHandle::read(void* buf, long len) {
    AutoLock<Mutex> lock(mutex_);
    if (fail_)
        throw Exception(std::string("MoverHandle::read: ") + error_);
...
long MoverHandle::write(const void* buf, long len) {
    AutoLock<Mutex> lock(mutex_);
    if (fail_)
        throw Exception(std::string("MoverHandle::write: ") + error_);
