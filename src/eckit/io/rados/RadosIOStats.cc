...
RadosIOStats::RadosIOStats(const std::string& prefix) :
    prefix_(prefix) {
...
RadosIOStats::~RadosIOStats() {}

void RadosIOStats::logMdOperation(const std::string& label, eckit::Timer& timer) {
...
void RadosIOStats::report(std::ostream& s) const {
    ...
    eckit::Timing t_rados;
    size_t numOps_rados = 0;
    double sumTimesSquared_rados = 0;
    ...
    for (const auto& x : md_stats_) {
        ...
        if (x.first.rfind("rados_", 0) == 0) {
            t_rados += x.second.timing;
            numOps_rados += x.second.numOps;
            sumTimesSquared_rados += x.second.sumTimesSquared;
        } else {
            t += x.second.timing;
    ...
    reportTimes(s, "total Rados API IO time", numOps_rados, t_rados, sumTimesSquared_rados);
    reportTimes(s, "total profiled eckit/Rados IO time", numOps, t, sumTimesSquared);
...
void RadosIOStats::reportCount(std::ostream& s, const std::string& label, size_t num) const {
...
void RadosIOStats::reportTimes(std::ostream& s, const std::string& label, size_t num, const eckit::Timing& sum,
