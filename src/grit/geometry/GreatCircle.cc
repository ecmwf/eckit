...
GreatCircle::GreatCircle(const Point2& Alonlat, const Point2& Blonlat) : A_(Alonlat), B_(Blonlat) {
    ...
    const bool lon_opposite = Apole || Bpole || util::approximately_equal(abs(lon12_deg), 180.);
...
std::vector<double> GreatCircle::latitude(double lon) const {
    ...
    const double lambda   = degrees_to_radians * util::normalise_longitude_to_minimum(B_[0] - A_[0], -180);
...
std::vector<double> GreatCircle::longitude(double lat) const {
    ...
    const double lon12 = degrees_to_radians * util::normalise_longitude_to_minimum(A_[0] - B_[0], -180);
