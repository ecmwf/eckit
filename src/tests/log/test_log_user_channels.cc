...
BOOST_AUTO_TEST_CASE(test_eckit_user_log_channel_registration) {
    ...
    BOOST_CHECK_NO_THROW( Log::registerChannel("mychannel", new Channel()) );

    BOOST_CHECK_NO_THROW( Log::channel("mychannel").setLogTarget( new ColouringTarget(new OStreamTarget(std::cout), &Colour::green)) );
