...
CASE("MARS grid") {
    ...
    SECTION("sh") {
        std::unique_ptr<const eckit::geo::Grid> g(eckit::geo::GridFactory::make_from_string("{grid: t1279}"));

        EXPECT("sh" == g->type());
        EXPECT(g->size() == 1639680);
        EXPECT_THROWS_AS(grid_type("{grid: t0}"), eckit::Exception);
    }
