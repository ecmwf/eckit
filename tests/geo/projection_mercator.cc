...
CASE("Mercator: spec_str, proj_str") {
    ...
    SECTION("spec_str") {
        EXPECT(proj1.spec_str() == R"({"figure":"earth","lat_ts":14,"type":"mercator"})");
        EXPECT(proj2.spec_str() == R"({"figure":"earth","lat_ts":14,"type":"mercator"})");
        EXPECT(proj3.spec_str() == R"({"figure":"earth","lon_0":-180,"type":"mercator"})");
    }
