...
b:
    ...
    EXPECT_THROWS_AS(nested.custom("a?"), exception::SpecNotFound);

    EXPECT(nested.custom("a")->has_custom("b"));
    EXPECT_NOT(nested.custom("a")->has_custom("b?"));
    EXPECT_THROWS_AS(nested.custom("a")->custom("b?"), exception::SpecNotFound);
...
CASE("Spec <- Custom") {
    ...
    SECTION("conversion (3)") {
        ...
        c.set("foo", two);
        EXPECT(c.has("foo"));
        EXPECT_THROWS_AS(c.get_int("foo"), exception::SpecNotFound);  // cannot access as int
        ...
        EXPECT_THROWS_AS(d.get_int("foo"), exception::SpecNotFound);     // cannot access as int
        EXPECT_THROWS_AS(d.get_double("foo"), exception::SpecNotFound);  // cannot access as real
