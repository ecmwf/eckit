...
CASE( "test great circles intersections" )
{
    ...
    auto is_approximately_equal_longitude = [](double lon1, double lon2, double epsilon = std::numeric_limits<double>::epsilon()) -> bool {
        while (lon2 < lon1) {
            lon2 += 360;
        }
        while (lon1 >= lon1 + 360) {
        ...
        return  is_approximately_equal(lon1, lon2, epsilon) ||
